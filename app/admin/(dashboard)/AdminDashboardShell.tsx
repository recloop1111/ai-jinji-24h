...
// 認可（admin/super_admin）はサーバ側 layout.tsx（getAdminUser→redirect）で確定済み。
// ここでは到達ガードを持たない（二重リクエスト回避）。ログアウトのみ supabase.auth.signOut() を行う。
const navigation = [
...
export default function AdminDashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const showBack = pathname !== '/admin' && pathname !== '/admin/dashboard'
  const handleBack = () => {
          ...
          <div className="flex items-center gap-1 min-w-0">
            ...
            {showBack && (
              <button
                ...
                title="前の画面へ戻る"
                aria-label="前の画面へ戻る"
                className="p-2 -ml-1 rounded-lg hover:bg-white/10 text-gray-400 hover:text-white transition-colors shrink-0"
              >
                <ArrowLeft className="w-5 h-5" />
              </button>
            )}
            <h2 className="text-lg font-semibold text-white truncate">{pageName}</h2>
          </div>
