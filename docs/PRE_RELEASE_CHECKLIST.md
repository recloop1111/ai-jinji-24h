...
### 0-9b. 受容リスク記録（backlog・コード変更なし）

- **questions_snapshot のクライアント送信（snapshot API）**: `POST /api/interview/[slug]/snapshot` は client が送る `questions_snapshot` をそのまま保存している（サーバ再取得していない）。現状の consumer は `/questions`（再取得時に snapshot を返す）のみで、**interview_results / EBCA評価 / 課金 / 合否 / 管理画面表示には使われず、応募者自身の面接表示に閉じる**ため受容（backlog）。**将来、EBCA writer や管理画面で questions_snapshot を「実施した質問の正」として信用する場合は、client 送信値ではなくサーバ側で再取得（`/questions` のサーバ導出ロジックを snapshot 生成にも使う）に変更すること。**

### 0-10. グループC前 全コードベース監査（案B / Claude実施・2026-06-26）

> PR差分でなく全コードベースを横断監査。詳細は PR #1 のコメント参照。**P0/P1 新規なし**。

- **検出・修正済み（P2・コードのみ）**: applicant 詳細ページ（admin/client）の無限再レンダーループ（`createXxxBrowserClient()` を useEffect 依存に入れていた）→ `useMemo` で安定化（`19156ba`・/client/billing `6620f8f` と同方式）。横断確認で**同型ループ残ゼロ**。
- **問題なし判定**: IDOR（client `[id]`/`[memo_id]`/`[template_id]` は全て `company_id` スコープ）／認証ガード網羅（admin=getAdminUser・client=getClientUser・interview=capability token・batch=INTERNAL_BATCH_SECRET）／課金サーバ算出・JST境界・孤児finalizeレース修正／SMS固定コードの本番fail-closed／service-roleはサーバのみ・ブラウザはanon／token堅牢／デモは本番無効。
- **バックログ（P3・後回し可）**: admin/companies のエラー文言一般化（管理者向けに内部message連結）／PII GET の no-store付与／内部バッチBearerの timing-safe比較／デモ経路コード削除（本番では無効）／応募者作成のIP rate limit追加。

### 0-11. Dashboard / env 人間確認メモ（2026-06-26・実値非記載）

> Supabase / Cloudflare / Vercel の**設定状態のみ**を記録（secret 実値・cookie値・キー文字列は記載しない）。本番デプロイ前に「要対応」を解消する。

| # | 項目 | 現状 | 本番前の要否 |
|---|------|------|------------|
| 1 | **Supabase Auth CAPTCHA** | **OFF**（Turnstile widget は作成済みだが Supabase 側で未有効化） | **要対応**: 本番前に ON＋Turnstile secret 設定。現状ログインの bot/総当り耐性は**スロットルのみ**（account10/IP60・サーバ強制は機能）。`captchaToken` は CAPTCHA 有効化までサーバ側で実質no-op |
| 2 | **Cloudflare Turnstile ホスト** | Preview ホスト登録済み（widget `ai-jinji-24h-preview` / Managed / solve実績あり）。**本番ドメイン未取得→本番ホスト未登録** | **後工程（必須）**: 本番ドメイン取得後に Turnstile Hostname へ本番ドメインを追加（`vercel.app` 等 public suffix は登録不可） |
| 3 | **Vercel env（secret系）** | `SUPABASE_SERVICE_ROLE_KEY` / `INTERVIEW_TOKEN_SECRET` / `AUTH_LOGIN_THROTTLE_SECRET` / `TURNSTILE_SECRET_KEY` / `NEXT_PUBLIC_TURNSTILE_SITE_KEY` / `SMS_FIXED_CODE_COMPANY_ID` は **Preview / ブランチのみ**。Production 未設定。`NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` は All Env。**NEXT_PUBLIC 誤設定なし** | **本番デプロイ前に必須**: 上記 secret 系を Production スコープで本番値設定（未設定だと fail-closed で 500/機能不全）。値はコミット禁止 |
| 4 | **SMS_FIXED_CODE_COMPANY_ID** | **Production 未設定**（Preview のみ・テスト企業） | **安全側でOK**: 本番で固定コード1234が無効＝望ましい。本番でも有効化する場合はテスト企業IDのみ |
| 5 | **INTERNAL_BATCH_SECRET** | Vercel で**見当たらず＝未設定の可能性** | **該当バッチ使用前に必須**: 未設定だと `/api/internal/batch/*` は 401 fail-closed でバッチ no-op（停止自動実行等が動かない）。使う前に設定 |
| 6 | **認証 cookie 属性（SameSite/Secure/HttpOnly）** | client cookie 観測: SameSite=Lax / Secure無 / HttpOnly無。→ **cookieOptions に `sameSite:'lax'` ＋ `secure: NODE_ENV==='production'` を明示**（`lib/supabase/{client,server,middleware}.ts`）。本番/Preview(https) で Secure=✓、dev(http) は無効で互換維持 | **Secure 対応済み（コード）**。Preview で Secure=✓ / SameSite=Lax を実機確認。admin cookie も同様の確認推奨 |

> **HttpOnly に関する設計判断（受容リスク・記録）**: 認証 cookie に **HttpOnly は付けない**。理由＝本アプリは `createClientBrowserClient` / `createAdminBrowserClient`（ブラウザ側 Supabase クライアント）で **authenticated な直接クエリ**を行い、`document.cookie` からセッションを読む構成のため。HttpOnly にすると JS が cookie を読めず client/admin のブラウザ直クエリが anon 化して壊れる。緩和策＝**SameSite=Lax ＋ Secure（本番）＋ 短命 access token**。**将来「ブラウザ直クエリを全廃しサーバAPI経由に統一」した時点で HttpOnly 化を再検討**する（現フェーズの作業ではない）。

**本番前 must（env/Dashboard）**: ① Supabase CAPTCHA を ON（＋Turnstile secret）／② 本番ドメイン取得後に Turnstile 本番ホスト登録／③ Production スコープへ secret 系 env を本番値で設定／④ バッチ使う場合は INTERNAL_BATCH_SECRET 設定／⑤ cookie 属性の確認。
**現状の安全側ポイント**: SMS_FIXED_CODE_COMPANY_ID は Production 未設定で 1234 が本番無効／NEXT_PUBLIC 誤設定なし／secret 系は未設定でも fail-closed。
...
> **導入計画は `docs/GROUP_C_PLAN.md` を正**（推奨導入順・必要env一覧・各APIの fail-closed/E2E・判断事項・プラン切替タイミング）。本節は E2E チェック項目の要約。
> 方針: 開発・Preview 検証は **Free/Hobby 維持**、**クライアント導入直前に Vercel Pro / Supabase Pro へ切替**し、本番ドメイン取得・Production env・Supabase CAPTCHA・Turnstile 本番ホスト・本番デプロイ・有料API接続をまとめて実施。
...
- [ ] **請求書PDF 発行者/振込先/登録番号のプレースホルダ**：`lib/config/billing.ts` の `BILLING_ISSUER`（会社名/住所/TEL/メール/代表者/**登録番号**）・`BILLING_BANK`（銀行/支店/口座種別/**口座番号**/名義）は現状ダミー値。**本番前に正式な実値へ必ず差し替える**（口座情報・適格請求書登録番号を含むため発行前必須）。登録番号が空のとき請求書PDFには「未登録」と表示される
- [ ] **本番前 console 撤去（QuestionEditor）**：`components/shared/QuestionEditor.tsx` の `console.error` 9箇所（L215/259/274/308/487/499/514/521/645）。token/secret は出力していないが本番前に撤去（CLAUDE.md「console を本番に残さない」方針）。修正は phase_e 確認後に別途判断（今回はコード未修正・記録のみ）
...
6. **請求書PDF 発行者/振込先/登録番号の実値差し替え**（3-2：`lib/config/billing.ts`）→ 口座情報・登録番号がダミーのまま発行されないようにする
