...
### 1-4. 社風アンケート（/survey/[slug]）
- [ ] `/survey/[slug]` が開き、会社名・部署・雇用形態が表示（`survey public-config`）
- [ ] 20問（7段階）＋自由記述に回答し送信できる（全問必須バリデーション）
- [ ] `response` API が 200、`culture_survey_responses` に保存される
- [ ] `culture_profiles` が response_count に応じて更新/作成される（count<3=件数のみ／count≥3=5因子平均）
- [ ] client/culture-analysis に社風プロファイルが反映される（authenticated・自社）

### 1-5. RLS 遮断確認（anon key で REST 直叩き）
- [ ] anon で `interview_results` / `applicants` / `interviews` / `companies` / `common_questions` / `jobs` / `job_questions` が 0件 or 403
- [ ] anon で `culture_surveys` / `culture_survey_responses` / `culture_profiles` が 0件 or 403（auth.uid() スコープで実効遮断）
...
- [ ] `GET /api/survey/[slug]/public-config` が安全列＋company{id,name}のみ
- [ ] `POST /api/survey/[slug]/response` が 200・サーバ側スコア計算で保存
...
### 後回しでよい
- admin/applicants[id] の culture_profiles 参照是正（admin_select_culture_profiles or admin用 service-role API）
- survey の重複回答/スパム対策（localStorage / cookie / IP rate limit / reCAPTCHA）
