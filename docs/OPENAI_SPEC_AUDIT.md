...
## Realtime モデル（現行）＋ AIMEN24 の選定
- **`gpt-realtime-2.1-mini`（R1 primary・本番標準候補・既定）**: WebRTC / function calling / prompt caching 対応。音声料金 **$10/1M in・$20/1M out**（2.1 比 約68%減）。reasoning effort は明記なし（送らない）。→ AIMEN24（1面接4,000円の従量課金）では「必要品質を満たす最小原価」を standard にする方針で primary 採用。
- **`gpt-realtime-2.1`（高品質 fallback）**: `gpt-realtime-2` を更新。alphanumeric 認識・**silence/noise handling・interruption behavior**・instruction following・tool use 改善。**configurable reasoning effort** 対応。音声料金 **$32/1M in・$64/1M out**。mini が R1 acceptance を満たさない場合のみ fallback。
- `gpt-realtime-2`、`gpt-realtime-1.5` も実在（allowlist に保持）。
...
## 本 patch での対応（コード）
- `REALTIME_DEFAULT_MODEL='gpt-realtime-2.1-mini'`（primary）、`REALTIME_FALLBACK_MODEL='gpt-realtime-2.1'`。`REALTIME_ALLOWED_MODELS` に mini/2.1/2/1.5 保持（deprecated gpt-realtime/mini は互換許可・非 default）、`REALTIME_DEPRECATED_MODELS` 明示。
