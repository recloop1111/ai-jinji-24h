...
- 公開面接フローの **browser Supabase 直書きを全廃**し、token付き **service-role API** へ移行（service-role は RLS bypass）。各APIは slug/applicant_id/company/interview の整合を再検証：
  ...
  - `POST /api/interview/[slug]/end` — 終了確定（interviews=completed/cancelled・ended_at・**is_billable＝正式課金仕様**〔completed は必ず課金／applicant_exit は duration>=180s かつ(main質問50%以上 or duration>=480s)／**time_limit（時間切れ＝面接時間上限まで提供）は必ず課金**／technical・system・forced・孤児は非課金。旧「10分超で課金」は廃止・superseded。**answered/total は server 解決**（total=`interviews.questions_snapshot` 件数／answered=`interviews.interview_progress.completedCount`〔R1-A・Prod 未適用のため現状 null→50%不使用・8分ルールのみ〕・client body は課金にも保存にも使わない）。純ロジック=`lib/billing/interview-eligibility.ts`〕・**applicants.status を end API 側で確定**：completed→'完了'・cancelled→'途中離脱'（**result は自動設定しない**＝途中離脱≠不採用。企業が後から判断）／他 in_progress を cancelled 化）
...
- billing 整理状況:
  ...
  - **課金フラグ列ドリフトを是正済み**: 実DB列は `interviews.is_billable`（旧 `billable` は不在）・`interviews.ended_at`（旧 `completed_at` は不在）。読み取りカウント側は `.eq('is_billable', true)` に統一。**is_billable の writer は `POST /api/interview/[slug]/end`（service-role）で確定**（純ロジック `lib/billing/interview-eligibility.ts`）。**課金判定（正式仕様・旧「10分超」INT-009 は廃止/superseded）**: 1 interview=最大1課金。**completed／time_limit（時間切れ＝面接時間上限まで正常提供→completed 相当）は必ず課金**／**applicant_exit（本人による途中離脱のみ）は duration>=180s かつ(main質問50%以上回答〔ceil〕 or duration>=480s)**／**technical/system/forced/孤児（本人 /end 未達）は非課金**。duration は server 権威（started_at 由来）。answered/total は server 解決（total=questions_snapshot 件数〔Realtime のみ freeze・mock は null〕／answered=interview_progress.completedCount〔R1-A・Prod 未適用かつ reducer 未結線＝現状 null → 8分ルールのみ〕・client body は課金にも保存にも使わない）。`/api/interview/[slug]/complete`・`/status` route は実DB列へ是正済みだが**呼び出しゼロの死蔵 endpoint**（削除は今回せず温存）
  - **Demo（テスト株式会社＝現在唯一の is_demo=true）の扱い（正式仕様）**: DB 権威 `companies.is_demo` のみで判定（client の is_demo/query/mode は不使用）。**「企業自身の利用表示」は demo でもカウント**（本番に近いデモ挙動を確認できる方が望ましい）＝ client plan の当月利用件数・admin companies〔一覧/[id]〕の当月利用件数は demo も実数表示。ただし **client plan の見込み請求額は demo=0**。**「運営の課金/売上/請求/全体集計」は demo を厳格除外**＝ `monthly-billing` batch（billing_record/invoice 作らず skippedDemo）・`admin/billing/summary`（billable 集計 companyIds から除外）。**月間上限は demo をブロックしない**（`interview start` の上限判定を demo でスキップ）。`interviews.is_billable` 自体は demo でも既存ルールどおり true/false（実請求には使わない）。純ロジック `lib/billing/demo-exclusion.ts`。応募者詳細の「利用計上」は demo tri-state（`/api/client/company-flags`・true→対象外/false→is_billable/**unknown→—**）。
  - **time_limit は server duration で検証（正式仕様）**: client が `end_reason='時間切れ'/'timeout'` と言っても、`/end` は server 実 duration が `MAX_INTERVIEW_SECONDS - 5s(tolerance)` に達していなければ time_limit へ昇格させず **4xx で拒否し finalize しない**（in_progress 維持）。偽の時間切れを別理由へ勝手に変換しない。
  - **end_reason allow-list**: `/end` は正式 domain 値（`lib/interview/end-reason.ts` `ALLOWED_END_REASONS`＝DB CHECK と一致）のみ許可。未知値は 4xx（DB CHECK 500 を未然防止）。
  - **【trust boundary / Public Production 前 blocker】** `final_status`/`end_reason` は依然 client-assert（server で完全証明できていない）。**completed authority = R1 で interview_progress migration 適用＋server progress writer 結線で裏付け**。**technical/disconnect authority = Public 前に server relay（Option B）で server 観測**。現状 completed/technical の client 詐称は完全には防げない（time_limit・demo 除外・duration・allow-list で緩和済み）。
