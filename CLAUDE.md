...
- billing 整理状況:
  ...
  - **Demo は課金・利用量から完全除外（正式仕様）**: DB 権威 `companies.is_demo=true` の企業は 請求件数/billing usage/月間利用件数/プラン上限消費/請求金額 に含めない（純ロジック `lib/billing/demo-exclusion.ts`）。除外は集計・上限側（monthly-billing／admin billing summary／admin companies〔一覧+[id]〕／client plan／interview start の上限）で `company.is_demo`（**DB のみ**・client の is_demo/query/mode は不使用）により実施。demo interview の `is_billable` 自体は既存フロー互換のため不変。
